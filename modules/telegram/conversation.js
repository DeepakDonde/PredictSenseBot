...
class TelegramEvent extends EventEmitter { }

const telegramEvent = new TelegramEvent();

/**
 * This is a intent message, discard old flow and continue new intent flow
 */
telegramEvent.on("intent_message", function (envelope) {
    console.log("Intent message");
    clearUserData(envelope.sender_id);
    startBotFlow(envelope);
})

telegramEvent.on("text_message", function (envelope) {
...
telegramEvent.on("attachment_message", function (sender_id, message) {
    console.log("attachment message is received");
});


function startBotFlow(envelope) {
    apiHelper.GetMessagesForIntent(config.CONVEE.BOT_ID, config.CONVEE.USER_ID, envelope.message.text, function (data) {
        if (data.BotIntentFlow && data.BotIntentFlow.length > 0) {
            kvs.set(envelope.sender_id + kvsTags.TAG_INTENT, data.IntentName);
            kvs.set(envelope.sender_id + kvsTags.TAG_ARRAY, data.BotIntentFlow);
            kvs.set(envelope.sender_id + kvsTags.TAG_MENU, data.MenuTrigger);
            ManageFlows(envelope);
        }
        else {
            if (data.MenuTrigger) {
                SendMenuTrigger(envelope.sender_id, data.MenuTrigger);
            }
            else {
                console.log("no data for user input");
                telegramController.sendTextMessage(envelope.sender_id, defaultMessages.NOT_SURE, 0);
            }
        }
    });
}
...
function continueBotFlow(envelope) {
    ...
    CheckMessageSendByUser(envelope, result_MessageArray, function (ValdiationResponse) {
        ...
        if (ValdiationResponse.StartMenuTrigger) {
            ...
            telegramController.sendTextMessage(envelope.sender_id, ValdiationResponse.ReturnMessage);
        }
        else {
...
function SendMenuTrigger(sender_id, resultMenuTrigger) {
    console.error("Tel:Sending menu trigger....");
    clearUserData(sender_id)
    if (resultMenuTrigger.MenuMessage) {
        telegramController.SendMenuTrigger(sender_id, resultMenuTrigger);
    } else {
        telegramController.sendTextMessage(sender_id, defaultMessages.NONE_INTENT_MESSAGE);
    }
...
function ManageFlows(envelope) {
    ...
    genericHelper.filterArray(MessageArray, function (FilteredMessageArray) {
        ...
        if (FilteredMessageArray.length > 0) {
            ...
            if (FilteredMessageArray[0].name === messageType.TIMEOUT) {
                waitForTimeout(envelope, FilteredMessageArray);
                return;
            }
        ...
        if (SentMessage) {
            ...
            if (SentMessage.name === 'Prompts') {
                ...
                if (ResultResponse.isValid) {
                    ...
                    //  telegramController.sendTextMessage(envelope.sender_id, defaultMessages.PROMPT_ACKNOWLEDGED, 0);
                }
 ...
 */
function waitForTimeout(envelope, FilteredMessageArray) {
    console.log("sending message for seconds-", FilteredMessageArray[0].duration);
    telegramController.sendTyping(envelope.sender_id, true);
    setTimeout(() => {
        FilteredMessageArray[0].isSent = 1;
        kvs.set(envelope.sender_id + kvsTags.TAG_ARRAY, FilteredMessageArray);
        // telegramController.sendTyping(envelope.sender_id,false);
        ManageFlows(envelope);
    }, FilteredMessageArray[0].duration * 1000);
        ...
        function (element, next) {
            if (!binded) {
                if (element.isSent === 0) {
                    ...
                    getEntityInputIfAny(envelope, function (entityInput) {
                        console.log("extracted entity",entityInput);
                        if (entityInput) {
                            element.answer = entityInput;
                        }
...
function getEntityInputIfAny(envelope,callback) {
    apiHelper.GetEntityForMessage(config.CONVEE.BOT_ID, config.CONVEE.USER_ID, envelope.message.text, function (data) {
        try {
            callback(data?data[0].entity:null);
        } catch (error) {
            console.error(error);
            callback(null);
        }
...
function SendMessage(type, envelope, FilteredMessageArray, MainMessageArray, resultMenuTrigger) {
    console.log("Send Telegram Messages")
    var DurationTime = 0;
    ...
    switch (type) {
        ...
        case messageType.TEXT:
            {
                telegramController.sendTextMessage(envelope.sender_id, FilteredMessageArray[0].data, DurationTime);
            }
            break;
        ...
        case messageType.BUTTON:
            {
                telegramController.sendButtonMessage(envelope.sender_id, kvs.get(envelope.sender_id + kvsTags.TAG_INTENT), FilteredMessageArray[0], DurationTime);
            }
            break;
        case messageType.IMAGE:
            {
                telegramController.sendImageMessage(envelope.sender_id, FilteredMessageArray[0].data, DurationTime);
            }
            break;

        case messageType.AUDIO:
            {
                telegramController.sendAudioMessage(envelope.sender_id, FilteredMessageArray[0].data, DurationTime);
            }
            break;
        case messageType.VIDEO:
            {
                telegramController.sendVideoMessage(envelope.sender_id, FilteredMessageArray[0].data, DurationTime);
            }
            break;
            ...
            if (item.attribute && item.attribute.length > 0) {
                ...
                for (var i = 0; i < item.attribute.length; i++) {
                    ...
                    if (attribute.isSent === 0) {
                        ...
                        telegramController.sendTextMessage(envelope.sender_id, attribute.question, 0);
                        break;
                    }
        ...
        case messageType.CAROUSEL:
            telegramController.sendTemplateMessage(envelope.sender_id, kvs.get(envelope.sender_id + kvsTags.TAG_INTENT), FilteredMessageArray[0], 0);
            break;
    }
...
function sendJSONAPIResponse(sender_id, item, responseCallback) {
    apiHelper.constructJSONAPIRequestAndGetResponse(item, (err, response) => {
        if (err) {
            ...
            telegramController.sendTextMessage(sender_id, response, 0);
            ...
            each(msgChunk,
                function (chunk, next) {
                    ...
                    setTimeout(function () {
                        telegramController.sendTextMessage(sender_id, chunk, 0);
                        next(null, chunk);
                    }, 1000);
...
function sendNoneIntentMessage(sender_id) {
    clearUserData(sender_id);
    telegramController.sendTextMessage(sender_id, defaultMessages.NONE_INTENT_MESSAGE, 1000);
}
...
function exitConversation(sender_id) {
    clearUserData(sender_id);
    telegramController.sendTextMessage(sender_id, defaultMessages.EXIT, 1000);
}

module.exports = telegramEvent;
