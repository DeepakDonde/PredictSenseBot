...
facebookEvent.on("text_message", function (envelope) {
    console.error("text message is received:" + envelope.message.text);
...
function SendMenuTrigger(sender_id, resultMenuTrigger) {
    console.error("FB:Sending menu trigger....");
    clearUserData(sender_id)

    if (resultMenuTrigger.MenuMessage) {
        facebookController.SendMenuTrigger(sender_id, resultMenuTrigger);
    } else {
        facebookController.sendTextMessage(sender_id, defaultMessages.NOT_SURE, 0);
    }
}
...
function waitForTimeout(envelope,FilteredMessageArray) {
    console.log("waitForTimeout-",FilteredMessageArray[0].duration);
    facebookController.sendTyping(envelope.sender_id,true);
    setTimeout(() => {
        FilteredMessageArray[0].isSent = 1;
        kvs.set(envelope.sender_id + kvsTags.TAG_ARRAY, FilteredMessageArray);
        facebookController.sendTyping(envelope.sender_id,false);
        ManageFlows(envelope);
